...
const codeGenerator = (node, options = {}) => {
    ...
    switch (node.type) {
        ...
        case 'ForStatement':
            return 'for (' + codeGenerator(node.init, { withExtraEndComa: true }) + ' ' + codeGenerator(node.test, { removeLeftExpression: true }) + ') {\n\t' + codeGenerator(node.body) + '\n}'

        case 'AssignmentExpression':
            return codeGenerator(node.left) + node.operator + codeGenerator(node.right) + (withExtraEndComa ? ';' : '')
        case 'BinaryExpression':
        case 'LogicalExpression':
            return (removeLeftExpression ? '' : '(' + codeGenerator(node.left)) + ' ' + node.operator + ' ' + codeGenerator(node.right) + (removeLeftExpression ? '' : ')')
